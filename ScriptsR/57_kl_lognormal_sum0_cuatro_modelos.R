...
# normales multivariantes (Espacio ILR) para los cuatro ajustes Stan
# H3917: Modelos Log-normal con coeficientes CLR de suma cero
###############################################################
...
output_dir <- file.path(data_dir, "resultados_kl_lognormal_sum0")
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

input_file <- file.path(data_dir, "stan_lognormal_sum0_fits.Rdata")
...
calc_kl_for_lognormal_model <- function(fit,
  ...
  # Obtener base ILR estándar correspondiente para proyectar las medias CLR
  V <- compositions::ilrBase(D = K)
...
# Reconstrucción dinámica de X_mat si falta en el entorno
if (!exists("X_mat")) {
  message("-> 'X_mat' no detectada en los ficheros .Rdata. Reconstruyendo dinámicamente...")
  if (exists("df.ancho")) {
...
fits_lognormal <- list()
if (exists("fit_lognor_v1_sum0coef")) fits_lognormal$lognor_v1_sum0coef <- fit_lognor_v1_sum0coef
if (exists("fit_lognor_v1_dia_sum0coef")) fits_lognormal$lognor_v1_dia_sum0coef <- fit_lognor_v1_dia_sum0coef
if (exists("fit_lognor_v1_sum0coef_delta_comun")) fits_lognormal$lognor_v1_sum0coef_delta_comun <- fit_lognor_v1_sum0coef_delta_comun
if (exists("fit_lognor_v1_dia_sum0coef_delta_comun")) fits_lognormal$lognor_v1_dia_sum0coef_delta_comun <- fit_lognor_v1_dia_sum0coef_delta_comun
...
summary_file <- file.path(output_dir, paste0("resumen_", suffix, "_lognormal_sum0.csv"))
