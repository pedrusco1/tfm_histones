...
calc_kl_for_lognormal_model <- function(fit,
  ...
  # La base ILR se pasa explícitamente desde las listas de datos guardadas
  # durante el ajuste. Los objetos stanfit de rstan no garantizan conservar
  # los datos originales en fit@stan_args[[1]]$data.
  V <- as.matrix(V)
  if (!all(dim(V) == c(K, K_ilr))) {
    stop(
      "Dimensiones incompatibles de V: se esperaba ", K, " x ", K_ilr,
      " y se obtuvo ", paste(dim(V), collapse = " x "), "."
    )
  }

  # Comprobaciones geométricas básicas de la base ILR.
  if (max(abs(crossprod(V) - diag(K_ilr))) > 1e-6) {
    stop("La matriz V no tiene columnas ortonormales.")
  }
  if (max(abs(colSums(V))) > 1e-6) {
    stop("Las columnas de V no pertenecen al subespacio CLR de suma cero.")
  }
...
# Recuperar exactamente la base ILR guardada durante el ajuste.
if (!exists("stan_data_lognor_v1_sin_dia_revisado")) {
  stop(
    "El archivo de ajustes no contiene `stan_data_lognor_v1_sin_dia_revisado`. ",
    "Vuelve a ejecutar 53_fit_lognor_sum0_model_revisado.R con la versión actual."
  )
}

if (is.null(stan_data_lognor_v1_sin_dia_revisado$V)) {
  stop("`stan_data_lognor_v1_sin_dia_revisado` no contiene la matriz V.")
}

V_lognor_revisado <- as.matrix(stan_data_lognor_v1_sin_dia_revisado$V)

# Los modelos con y sin día deben usar exactamente la misma base ILR.
if (exists("stan_data_lognor_v1_con_dia_revisado")) {
  V_dia <- as.matrix(stan_data_lognor_v1_con_dia_revisado$V)
  if (!isTRUE(all.equal(V_lognor_revisado, V_dia, tolerance = 1e-12))) {
    stop("Los modelos con y sin día no fueron ajustados con la misma matriz V.")
  }
}
