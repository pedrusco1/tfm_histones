...
## Modelo regresion de Dirichlet clasica
...
Sea $Y_n = (Y_{n1}, \dots, Y_{nK})$ la composición observada en la
muestra $n$ y $x_n$ la fila correspondiente de la matriz de diseño $X$;
se asume

$$Y_n \mid \boldsymbol{\alpha}_n \sim \mathrm{Dirichlet}(\boldsymbol{\alpha}_n),
\qquad
\boldsymbol{\alpha}_n = \phi_n \boldsymbol{\mu}_n,
\qquad
\boldsymbol{\mu}_n = \mathrm{softmax}(\boldsymbol{\eta}_n).$$

El predictor lineal por componente se define como
$$\eta_{n1} = 0, \qquad
\eta_{nk} = x_n^\top \boldsymbol{\beta}_k
...
referencia, mientras que la precisión $\phi_n$ controla la dispersión
global alrededor de la media composicional $\boldsymbol{\mu}_n$.
...
La media teórica de las proporciones de cada las $K$ PTM, se describe
...
$$ Y_n \sim \text{Dirichlet}(\alpha_n)$$,

con $\alpha_n = \kappa_c \cdot \pi_c + \varepsilon$, donde $c$ indica la
condición experimental de la muestra, $\kappa_c$ es un parámetro de
concentración y $\varepsilon$ es una constante pequeña para evitar
...
siendo $\Delta = B^{\top} B$ representa el Laplaciano de la epistasia.
Esta matriz actúa como un operador de curvatura discreta: no penaliza
los campos puramente aditivos, pero sí las desviaciones locales respecto
a la aditividad.
...
\beta, \Omega^{-1}),\quad \Omega = \tau \text{L}_{shape} + \delta I$$
...
epistática impuesta por $\tau \text{L}_{shape}$ [@wang].

## Medidas resumen de epistasia

A partir del campo de potenciales $f$ y del operador epistático $L$,
...
modelos de múltiples loci, donde se evalúa el peso de cada gen por
separado y cómo se alteran sus frecuencias cuando los genes se heredan
juntos de forma no aleatoria (desequilibrio de ligamiento). Finalmente,
se cuantifica el impacto de cada nivel calculando las energías $E^{(k)}$
para cada orden $k$ [@phillips2008; @álvarez-castro2019]. A partir de
ellas definimos índices como:
...
\approx \frac{1}{M} \sum_{m=1}^M \mathbb{I}\bigl(\theta_g^{(m)} >
...
de
$\mathrm{KL}(\mathrm{Dir}(\boldsymbol{\alpha}) \,\|\, \mathrm{Dir}(\boldsymbol{\beta}))$
viene dada porla función digamma [@nawa2024].
...
Para la realización de este trabajos se han utlizado las siguentes
herramientas de inteligencia arificacial:
...
enlace \textcolor{blue}{url:: de github}.
