// -----------------------------------------------------------------------------
// Modelo: epistasia_kappa_cond_v2
// Propósito:
//   Ajusta un modelo Dirichlet para composiciones Y con dos condiciones
//   experimentales. Cada condición tiene su propio campo energético phi, su
//   propia suavidad epistática a y su propia concentración Dirichlet kappa[c].
//
// Diferencia frente a epistasia_kappa_comun:
//   Aquí kappa no es común; se estima un valor independiente por condición.
//   Esto permite que async y mitosis tengan diferente dispersión/concentración
//   alrededor de sus medias composicionales Q_c.
//
// Estructura estadística:
//   phi_c     = campo energético centrado para la condición c
//   Q_c       = softmax(-phi_c)
//   kappa[c]  = exp(log_kappa[c])
//   alpha     = kappa[c] * Q_c + 1e-6
...
// -----------------------------------------------------------------------------
//Modelo epistasia kappa por condicion
  data {
    ...
    matrix[K, K] Delta;          // operador mínima epistasia, similar laplaciono 2-orden
  ...
  model {
    ...
    // Likelihood Dirichlet, solo si prior_only=0
    if (prior_only == 0){
      for (n in 1:N) {
        int c = cond[n];
       vector[K] alpha = kappa[c] * Q[, c] + 1e-6;
